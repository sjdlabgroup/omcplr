    ...
    def sample_dna_breaks(self, genome_config):
        ...
        for chrom, size in genome_config.items():
            ...
            for _ in range(num_breaks):
                ...
                coord = coord + random.randint(100, 10000)
                self.chromosomes[chrom].append(coord)

    def process_sv_events(self, driver_prob):
        """Processes chromosomes with >= 2 breaks to trigger structural variants (SVs)."""
...
def load_inputs(bed_path, rates_path):
    ...
    # --- FIX CRITICAL TYPE CASTING HERE ---
    rates = {}
    for _, row in rates_df.iterrows():
        param = row['parameter']
        val = row['value']
        try:
            # Try converting numbers (like 0.25, 30, 100000) to numeric types
            rates[param] = float(val)
        except ValueError:
            # If it's a word/string (like 'final_sim_cell_svs' or 'True'), keep it as a string
            rates[param] = str(val)
...
def export_per_cell_beds(population, output_dir, t=None):
    ...
    if t is not None:
        print(f"Time {t:02d}: Population size = {len(population)} | Exported {cells_written} per-cell events to the folder: {output_dir}.")
    else:
        print(f"Final Step: Population size = {len(population)} | Exported {cells_written} per-cell events to the folder: {output_dir}.")


def run_simulation(bed_path, rates_path, output_bed_path="simulated_alterations.bed"):
    # Load configuration and simulation engine variables directly from parameters file
    ...
    print(f"Config details -> Max Steps: {max_time_steps} | Max Cells: {max_cells} | Report Daily: {report_all_timepoints}")
    ...
    for t in range(1, max_time_steps + 1):
        ...
        for cell in population:
            ...
            if random.random() < cell.rates.get('birth_rate', 0.2):
                ...
                daughter.process_sv_events(driver_prob)

                next_generation.append(daughter)

        population = next_generation

        # If option is True, export the current population tracks instantly at this specific time step
        if report_all_timepoints:
            timepoint_dir = f"{t}_{output_dir}"
    ...
    # 2. Export final individual cell maps if not writing every step intermediate data
...
if __name__ == "__main__":
    # Parameters are completely abstracted from parameters.txt (the rates_path argument)
    run_simulation(
