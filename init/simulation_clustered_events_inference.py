...
def prepare_noisy_directory(src_dir, dest_dir="tmp/tmp_cplxev", noise_factor=0.0, dropout_factor=0.0):
    """
    Copies files from `src_dir` to `dest_dir`. Replaces a given fraction
    defined by (noise_factor) of files with contents from other randomly selected files.
    Applies a (dropout_factor) to randomly drop individual entries (lines) during the copy.
    """
    ...
    if dropout_factor > 0:
        print(f"Applying dropout factor {dropout_factor}: Genomic events have a {dropout_factor*100:.1f}% chance of being dropped.")

    for idx, src_file_path in enumerate(all_files):
        filename = os.path.basename(src_file_path)
        dest_file_path = os.path.join(dest_dir, filename)

        # Determine which file's contents we are using (apply noise_factor)
        if idx in noisy_indices and total_files > 1:
            ...
            source_to_read = random.choice(donor_candidates)
        else:
            source_to_read = src_file_path

        # Apply dropout_factor while writing to the destination directory
        if dropout_factor > 0.0:
            with open(source_to_read, 'r') as f_in, open(dest_file_path, 'w') as f_out:
                for line in f_in:
                    # Drop line with probability == dropout_factor
                    if random.random() >= dropout_factor:
                        f_out.write(line)
        else:
            # If dropout is 0, just do a fast file copy
            shutil.copy2(source_to_read, dest_file_path)

    return dest_dir


def find_all_overlapping_svs_with_precise_breakdowns(
    input_dir="tmp/tmp_cplxev",
    ...
    if not all_records:
        print("No structural variant data found inside the files. Exiting!")
        return
...
if __name__ == "__main__":
    ...
    # Validate positional arguments (requires 5 arguments now)
    if len(sys.argv) < 6:
        print(f"Usage: python {script_name} [input_folder] [output_report.txt] [min_cells] [noise_factor] [dropout_factor]")
        print(f"Example: python {script_name} 10_sim_run output_report.tsv 1 0.2 0.05")
        sys.exit(1)
    ...
    try:
        dropout_factor = float(sys.argv[5])
        if not (0.0 <= dropout_factor <= 1.0):
            raise ValueError
    except ValueError:
        print("Error: dropout_factor must be a fraction between 0.0 and 1.0.")
        sys.exit(1)

    # 1. Copy files and apply noise & dropout substitutions to the destination folder
    working_dir = prepare_noisy_directory(
        src_dir=source_input_dir,
        dest_dir="tmp/tmp_cplxev",
        noise_factor=noise_factor,
        dropout_factor=dropout_factor
    )

    # 2. Run analysis on the newly populated 'tmp/tmp_cplxev' directory
